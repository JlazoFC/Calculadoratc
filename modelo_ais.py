...
def ejecutar_modelo_ais(nombre_muestra=None):
    ...
    # Configurar repositorios y opciones de instalación
    ...
    install_with_retry <- function(pkg, max_attempts = 3) {{
        for (attempt in 1:max_attempts) {{
            cat("Intento", attempt, "de instalar", pkg, "\n")
            tryCatch({{
                if (!require(pkg, character.only = TRUE, quietly = TRUE)) {{
                    ...
                    cat("✅", pkg, "ya está disponible\n")
                    return(TRUE)
                }}
            }}, error = function(e) {{
                cat("❌ Error en intento", attempt, "para", pkg, ":", e$message, "\n")
                if (attempt == max_attempts) {{
                    stop("No se pudo instalar ", pkg, " después de ", max_attempts, " intentos")
                }}
                Sys.sleep(2)
                return(FALSE)
            }})
    ...
    required_packages <- c("Matrix", "xgboost", "dplyr", "fastDummies")
    ...
    try:
        ...
        # Configurar variables de entorno para R
        env = os.environ.copy()
        env['R_MAX_VSIZE'] = '8Gb'
        env['R_LIBS_USER'] = os.path.expanduser('~/R/library')
        
        process = subprocess.Popen(['Rscript', '--vanilla', '--max-ppsize=500000', r_script_path],
                                env=env,
